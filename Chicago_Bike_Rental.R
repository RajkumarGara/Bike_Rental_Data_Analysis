...
#Model1: Deep Neural Network to predict the type of bike for the given station name
# Building a Neural network model
pred1_data <- cleaned_data %>% select(rideable_type, start_station_name)
pred1_data <- pred1_data[1:70000,]

start_station_name_vect <- unique(pred1_data$start_station_name)
rideable_type_vect <- unique(pred1_data$rideable_type)

pred1_data$start_station_name = factor(pred1_data$start_station_name, 
                                       levels = start_station_name_vect, 
                                       labels = c(1:length(start_station_name_vect)))

pred1_data$rideable_type = factor(pred1_data$rideable_type, 
                                       levels = rideable_type_vect, 
                                       labels = c(1:length(rideable_type_vect)))

pred1_data$start_station_name = as.numeric(pred1_data$start_station_name)
pred1_data$rideable_type = as.numeric(pred1_data$rideable_type)
...
xtest$start_station_name = as.numeric(xtest$start_station_name)

nn <- neuralnet(rideable_type ~ start_station_name, data = train, hidden = 2)
nn

# Predicting the values for train dataset
yy1 = predict(nn, train)
print(head(yy1))

# Predicting the values for test dataset
ypredict = predict(nn, xtest)
print(head(ypredict))
